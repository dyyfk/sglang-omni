...
- ``adaptive``: the quantized arm plus ``enable_adaptive_dispatch`` — the
  configured wait/floor engage only at >= ``--adaptive-threshold`` active
  streams with batched graphs live, else zero-wait (#1236 policy)
...
gate), fire-reason counts, and per-forward execution mode. Failed requests
invalidate the run rather than being dropped.
...
ARM_CHOICES = ("serial-eager", "serial-graph", "batched", "quantized", "adaptive")
...
def build_model_context(args: argparse.Namespace) -> ModelContext:
    ...
    graph_runner = None
    if any(arm in args.arms for arm in ("serial-graph", "quantized", "adaptive")):
        ...
        if "quantized" in args.arms or "adaptive" in args.arms:
...
def build_scheduler(
    ...
    return InstrumentedCode2Wav(
        ...
        enable_batching=arm in ("batched", "quantized", "adaptive"),
...
def build_stream_plans(
    ...
    *,
    ...
    frames = args.windows * args.stream_chunk_size
...
def run_single(
    ...
    *,
    ...
    record: dict[str, Any] = {
        ...
        "batch_floor": floor,
        "batch_ceiling": (
            args.ceiling if arm in ("batched", "quantized", "adaptive") else None
        ),
        "adaptive_threshold": (
            args.adaptive_threshold if arm == "adaptive" else None
        ),
        "repeat": repeat,
        "errors": clock.errors,
    }
    ...
    for entry in scheduler.step_log:
        ...
        if entry.get("adaptive_regime"):
            regimes[entry["adaptive_regime"]] = (
                regimes.get(entry["adaptive_regime"], 0) + 1
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="active-stream count at which the adaptive arm engages the wait",
    )
    ...
    if args.fake and any(
        arm in args.arms for arm in ("serial-graph", "quantized", "adaptive")
    ):
        parser.error("--fake cannot run the serial-graph, quantized or adaptive arms")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "adaptive" in args.arms:
        import sglang_omni.models.qwen3_omni.components.code2wav_scheduler as _sched

        _sched._ADAPTIVE_WAIT_MIN_ACTIVE_STREAMS = args.adaptive_threshold
