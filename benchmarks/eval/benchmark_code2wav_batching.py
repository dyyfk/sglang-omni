...
class InstrumentedCode2Wav(Code2WavScheduler):
    ...
    def _next_message(self):
        ...
        # Pre-b90d859 wake: the drain-and-coalesce block is the only reader
        # of _can_batch_stream_chunks inside _next_message, so clearing the
        # flag just for this call skips it; once restored, _handle_message
        # still routes chunks through _collect_stream_chunk_batch ->
        # on_stream_chunk_batch, keeping the wait/floor pump policy live.
...
def run_single(
    ...
    *,
    ...
    if clock.first_stream and clock.result:
        # All-streams-active window: last first-emission -> first completion.
        # Interior stream chunks are exactly one window of audio each; the
        # trimmed first chunk sits at/before t0 and flush tails at/after t1.
